...
- [ ] Implement runtime values, evaluator, and hermetic builtins for core language (completed: basic values + range type + expression evaluator + function calls + module execution + function bodies + loops + len/range/list/tuple/bool/any/all builtins + list/tuple/dict equality with hashability checks; remaining: full value model and builtins).
